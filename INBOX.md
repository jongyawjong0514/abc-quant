...
```yaml
role: technical_lead
task: "Add a command-line entry point for deterministic OLS smoke diagnostics."
target_files_or_folders:
  - "src/abc_quant/cli/linear_regression_smoke.py"
  - "src/abc_quant/cli/__init__.py"
  - "tests/test_cli_linear_regression_smoke.py"
  - "docs/modeling.md"
  - "README.md"
  - "STATUS.md"
  - "OUTBOX.md"
  ...
  - "TODO.md"
  - "INBOX.md"
current_spec_or_decision: "PR #35 centralized validation for linear regression smoke summaries. The next safe usability step is a thin CLI wrapper that prints the existing deterministic OLS diagnostics as JSON."
constraints:
  - "Keep this round limited to a CLI wrapper around run_linear_regression_smoke(...)."
  - "Use only Python standard library plus existing project code."
  - "Support module execution while preserving existing modeling, preprocessing, and supervised CLI behavior."
  - "Do not change OLS calculations, summary keys, split defaults, or existing smoke outputs."
  - "The command writes JSON only to stdout and concise errors only to stderr."
acceptance_criteria:
  - "Create src/abc_quant/cli/linear_regression_smoke.py with main(argv=None) -> int."
  - "Support python -m abc_quant.cli.linear_regression_smoke."
  ...
  - "Tests parse stdout JSON and verify it matches run_linear_regression_smoke(...)."
  - "Tests verify output is deterministic across repeated module calls."
  - "Tests verify custom split arguments pass through and alter split_counts_after_label_drop deterministically."
  - "Tests verify output contains only LINEAR_REGRESSION_SMOKE_SUMMARY_KEYS."
  - "Existing modeling, preprocessing, and supervised CLI tests still pass."
...
review_notes_or_defects:
  - "This task follows PR #35 and exposes OLS diagnostics only."
