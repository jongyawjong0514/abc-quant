...
- 新增 Task 047 LightGBM dependency smoke module CLI：`python -m abc_quant.cli.lightgbm_dependency_smoke` 以 sorted JSON 輸出既有 `run_lightgbm_dependency_smoke()` summary，支援 `--indent`，不新增 packaged console-script alias、不 require 真套件、不呼叫預設 `require_lightgbm()`、不 fit/search/select model、不產生 strategy/allocation/performance/order/position/simulation outputs。
