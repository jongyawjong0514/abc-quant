        ...
        rows.append(
            {
                ...
                "kd_k9": _float(row.get("kd_k9")),
                "kd_d9": _float(row.get("kd_d9")),
                "kd_oversold_marker": bool(row.get("kd_oversold_marker", False)),
                "kd_recent_oversold": bool(row.get("kd_recent_oversold", False)),
                "kd_k_rising": bool(row.get("kd_k_rising", False)),
...
def _stock_report(result: ZhuWalklineResult) -> str:
    ...
    lines = [
        ...
        f"| 均線 | {ma_state} |",
        f"| K棒 | {kline_state} |",
        f"| KD | K={_format_number(row.get('kd_k9'))}、D={_format_number(row.get('kd_d9'))}、階段={_text(row.get('kd_observation_stage', ''))} |",
        f"| 量能 | {volume_state} |",
        ...
        "### KD(9,3,3) 超賣轉強觀察",
        "",
        "同學，短線進入超賣區，不等於止跌。超賣只是位置標記，不是轉強訊號。",
        "後面還要等K向上、K突破D、價格站回壓力，而且個股仍屬多頭與強勢結構，才是另一層轉強確認。",
        "",
        f"- K值：{_format_number(row.get('kd_k9'))}",
        f"- D值：{_format_number(row.get('kd_d9'))}",
        f"- 當日超賣標記：{bool(row.get('kd_oversold_marker', False))}",
        f"- 近5日曾超賣：{bool(row.get('kd_recent_oversold', False))}",
        f"- K向上：{bool(row.get('kd_k_rising', False))}",
        ...
        f"- KD觀察階段：{_text(row.get('kd_observation_stage', ''))}",
        f"- KD轉強確認：{bool(row.get('kd_recovery_confirmation', False))}",
        "",
        "## 五、量能",
        "",
