"""Deterministic walk-forward supervised dataset smoke diagnostics."""
...
DEFAULT_WALK_FORWARD_SUPERVISED_MIN_TRAIN_SIZE: Final[int] = 4
DEFAULT_WALK_FORWARD_SUPERVISED_VALIDATION_SIZE: Final[int] = 2
DEFAULT_WALK_FORWARD_SUPERVISED_TEST_SIZE: Final[int] = 2
DEFAULT_WALK_FORWARD_SUPERVISED_STEP_SIZE: Final[int | None] = None
DEFAULT_WALK_FORWARD_SUPERVISED_MAX_WINDOWS: Final[int | None] = None

WALK_FORWARD_SUPERVISED_SMOKE_SUMMARY_KEYS: Final[tuple[str, ...]] = (
    "observation_count",
    "feature_columns",
    "label_column",
    "plan",
...
WALK_FORWARD_SUPERVISED_SMOKE_PLAN_KEYS: Final[tuple[str, ...]] = (
    "min_train_size",
    "validation_size",
    "test_size",
    "step_size",
...
WALK_FORWARD_SUPERVISED_SMOKE_WINDOW_KEYS: Final[tuple[str, ...]] = (
    "window_id",
    "index_ranges",
...
WALK_FORWARD_SUPERVISED_SMOKE_SPLITS: Final[tuple[str, ...]] = (
    "train",
    "validation",
    "test",
)
WALK_FORWARD_SUPERVISED_SMOKE_INDEX_RANGE_KEYS: Final[tuple[str, ...]] = (
    "start",
    "end",
)
WALK_FORWARD_SUPERVISED_SMOKE_FORBIDDEN_KEYS: Final[frozenset[str]] = frozenset(
    {
        "raw_features",
        "raw_feature_values",
        "raw_labels",
...
def run_walk_forward_supervised_smoke(
    *,
    min_train_size: int = DEFAULT_WALK_FORWARD_SUPERVISED_MIN_TRAIN_SIZE,
    validation_size: int = DEFAULT_WALK_FORWARD_SUPERVISED_VALIDATION_SIZE,
    test_size: int = DEFAULT_WALK_FORWARD_SUPERVISED_TEST_SIZE,
    step_size: int | None = DEFAULT_WALK_FORWARD_SUPERVISED_STEP_SIZE,
    max_windows: int | None = DEFAULT_WALK_FORWARD_SUPERVISED_MAX_WINDOWS,
) -> dict[str, Any]:
    """Run deterministic walk-forward supervised data diagnostics only.

    The helper builds in-memory split metadata from the deterministic smoke
    frame, fits scaling parameters separately inside each window using that
    window's train rows, and reports supervised row-count diagnostics. It does
    not execute estimators or produce downstream action artifacts.
    """
    ...
    summary = {
        ...
        "windows": [_window_summary(feature_matrix, window) for window in plan.windows],
    }
    return validate_walk_forward_supervised_smoke_summary(summary)


def validate_walk_forward_supervised_smoke_summary(
    summary: object,
) -> dict[str, Any]:
    """Validate the walk-forward supervised diagnostics summary shape."""
    if not isinstance(summary, dict):
        raise TypeError("walk-forward supervised smoke summary must be a dict")

    forbidden = sorted(
        WALK_FORWARD_SUPERVISED_SMOKE_FORBIDDEN_KEYS & _collect_nested_keys(summary)
    )
    if forbidden:
        raise ValueError(
            "walk-forward supervised smoke summary contains forbidden keys: "
            + ", ".join(forbidden)
        )

    _validate_key_tuple(
        "walk-forward supervised smoke summary",
        actual_keys=summary.keys(),
        expected_keys=WALK_FORWARD_SUPERVISED_SMOKE_SUMMARY_KEYS,
    )

    if not isinstance(summary["feature_columns"], list):
        raise ValueError("walk-forward supervised smoke feature_columns must be a list")
    if not isinstance(summary["windows"], list):
        raise ValueError("walk-forward supervised smoke windows must be a list")
    if not summary["windows"]:
        raise ValueError("walk-forward supervised smoke windows must not be empty")

    plan = summary["plan"]
    if not isinstance(plan, dict):
        raise ValueError("walk-forward supervised smoke plan must be a dict")
    _validate_key_tuple(
        "walk-forward supervised smoke plan",
        actual_keys=plan.keys(),
        expected_keys=WALK_FORWARD_SUPERVISED_SMOKE_PLAN_KEYS,
    )

    window_count = plan["window_count"]
    if not isinstance(window_count, int) or isinstance(window_count, bool):
        raise ValueError("walk-forward supervised smoke plan window_count must be int")
    if window_count != len(summary["windows"]):
        raise ValueError("walk-forward supervised smoke plan window_count mismatch")
    ...
    try:
        ...
        raise ValueError(
            "walk-forward supervised smoke summary must be JSON-friendly"
        ) from exc
...
def _validate_window_summary(window: object) -> None:
    if not isinstance(window, dict):
        raise ValueError("walk-forward supervised smoke window summaries must be dicts")
    _validate_key_tuple(
        "walk-forward supervised smoke window",
        actual_keys=window.keys(),
        expected_keys=WALK_FORWARD_SUPERVISED_SMOKE_WINDOW_KEYS,
    )
    ...
    scaler_feature_count = window["scaler_feature_count"]
    if (
        not isinstance(scaler_feature_count, int)
        or isinstance(scaler_feature_count, bool)
        or scaler_feature_count <= 0
    ):
        raise ValueError(
            "walk-forward supervised smoke scaler_feature_count must be positive int"
        )


def _validate_split_ranges(index_ranges: object) -> None:
    if not isinstance(index_ranges, dict):
        raise ValueError("walk-forward supervised smoke index_ranges must be a dict")
    _validate_key_tuple(
        "walk-forward supervised smoke index_ranges",
        actual_keys=index_ranges.keys(),
        expected_keys=WALK_FORWARD_SUPERVISED_SMOKE_SPLITS,
    )
    for split_name in WALK_FORWARD_SUPERVISED_SMOKE_SPLITS:
        ...
        _validate_key_tuple(
            ...
            expected_keys=WALK_FORWARD_SUPERVISED_SMOKE_INDEX_RANGE_KEYS,
        )
        for key in WALK_FORWARD_SUPERVISED_SMOKE_INDEX_RANGE_KEYS:
...
def _validate_split_count_mapping(window: dict[str, Any], key: str) -> None:
    ...
    if not isinstance(split_counts, dict):
        raise ValueError(f"walk-forward supervised smoke {key} must be a dict")
    _validate_key_tuple(
        f"walk-forward supervised smoke {key}",
    ...
    for split_name in WALK_FORWARD_SUPERVISED_SMOKE_SPLITS:
        ...
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise ValueError(
                f"walk-forward supervised smoke {key} {split_name} "
                "must be nonnegative int"
            )
